...
Use Elixir/OTP stdlib directly from `elixir_eval` for ordinary runtime, file, and process work. Reach for `Pi.*` shortcuts only when they provide bounded summaries or remove repetitive boilerplate. For bridge self-introspection, prefer the preloaded `Self` alias (`Pi.Self`) for `Self.status()`, `Self.quack()`, `Self.bindings()`, `Self.sessions()`, and `Self.context(query)`. For semantic code reflection, prefer the preloaded `CodeMap` alias (`Pi.CodeMap`) for `CodeMap.reflect(changed: true)`, `CodeMap.context(target)`, `CodeMap.hotspots(path: file)`, and `CodeMap.smells(path: file)`. For session history analytics, prefer the preloaded short aliases `Q` (`Pi.Quack`), `E` (`Pi.Quack.Event`), and `SF` (`Pi.Quack.SessionFile`) with normal `Ecto.Query`/`QuackDB.Ecto` DSL, then render via `Q.table()` or `Pi.table()`. For docs/source context, prefer pipelines such as `Pi.Docs.module(Mod) |> Pi.Docs.functions() |> Pi.Docs.search("query")` and `Pi.Docs.module(Mod) |> Pi.Docs.function(:name, arity) |> Pi.Docs.source(context: 25)`. For simple web context, prefer bounded `Pi.Web.fetch!/2` over raw `Req`.
